...
def build_dataloaders():
    ...
    tokenized_dataset = dataset.map(
        ...
        batched=True,
        num_proc=CFG.NUM_CPU // 2, # alternatively you can set the value to 1/2 or anything lower
        remove_columns=["text"],
    )
    ...
    train_dataset = tokenized_dataset.map(
        group_texts, batched=True, num_proc=CFG.NUM_CPU // 2, # alternatively you can set the value to 1/2 or anything lower
    )
...
def main():
    ...
    # 1B
    model = PaLM(
        num_tokens=50304, # set token to a lower value for less memory utilisation
        dim=2048,
        depth=16,
        dim_head=128,
        heads=8,
    ...
    sharding_strat = ShardingStrategy.NONE

    model = FullyShardedDataParallel(
        model,
        mixed_precision=bf16_fsdp,
        backward_prefetch=BackwardPrefetch.UNFETCHED,
    ...
    train_loader = DataLoader(
        train_dataset, batch_size=CFG.BATCH_SIZE, collate_fn=default_data_collator, pin_memory=False
    )
